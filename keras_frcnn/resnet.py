...
def identity_block(input_tensor, kernel_size, filters, stage, block, trainable=True):
    ...
    x = Activation('relu')(x)
    tf.summary.histogram("identity_block_weight",nb_filter1)		#Baron added
...
def nn_base(input_tensor=None, trainable=False):
    ...
    x = identity_block(x, 3, [256, 256, 1024], stage=4, block='g', trainable = trainable) #Baron added
    x = identity_block(x, 3, [256, 256, 1024], stage=4, block='h', trainable = trainable) #Baron added
    x = identity_block(x, 3, [256, 256, 1024], stage=4, block='i', trainable = trainable) #Baron added
...
def classifier_layers(x, input_shape, trainable=False):
    ...
    x = identity_block_td(x, 3, [512, 512, 2048], stage=5, block='d', trainable=trainable) #Baron added
    x = identity_block_td(x, 3, [512, 512, 2048], stage=5, block='e', trainable=trainable) #Baron added
    x = identity_block_td(x, 3, [512, 512, 2048], stage=5, block='f', trainable=trainable) #Baron added
